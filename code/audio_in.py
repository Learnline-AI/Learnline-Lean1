...
class AudioInputProcessor:
    ...
    # Railway optimization constants
    _RAILWAY_DEPLOYMENT = os.getenv("RAILWAY_DEPLOYMENT", "false").lower() == "true"
    _RAILWAY_CHUNK_TIMEOUT = 0.01 if _RAILWAY_DEPLOYMENT else 0.005  # Slower processing for Railway
    ...
    async def _run_transcription_loop(self) -> None:
        ...
        while True: # Loop restored to continuously call transcribe_loop
            try:
                ...
                # Railway-optimized sleep to prevent tight loop and reduce CPU usage
                await asyncio.sleep(self._RAILWAY_CHUNK_TIMEOUT)
